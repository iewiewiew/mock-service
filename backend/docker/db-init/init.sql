...
-- 项目示例数据
INSERT INTO `test_platform`.`projects` (name,description,created_at,updated_at) VALUES ('示例项目1','',NOW(),NOW());
INSERT INTO `test_platform`.`projects` (name,description,created_at,updated_at) VALUES ('示例项目2','',NOW(),NOW());

-- Mock接口示例数据
INSERT INTO `test_platform`.mocks (name,`path`,`method`,response_status,response_body,description,project_id,created_at,updated_at) VALUES ('GET接口示例','/api/users','GET',200,'{"users": [{"id": 1}]}','','1',NOW(),NOW());
INSERT INTO `test_platform`.mocks (name,`path`,`method`,response_status,response_body,description,project_id,created_at,updated_at) VALUES ('POST接口示例','/api/users','POST',201,'{"request_id":"{request.json.id}","req_str":"{request.json.req_str}","action":"{request.args.action}","email":"${email}","example":"示例"}','','1',NOW(),NOW());
INSERT INTO `test_platform`.mocks (name,`path`,`method`,response_status,response_body,description,project_id,created_at,updated_at) VALUES ('PUT接口示例','/api/users/1','PUT',200,'{"request_id":"{request.json.id}","req_str":"{request.json.req_str}","action":"{request.args.action}","email":"${email}","example":"示例"}','','1',NOW(),NOW());
INSERT INTO `test_platform`.mocks (name,`path`,`method`,response_status,response_body,description,project_id,created_at,updated_at) VALUES ('DELETE接口示例','/api/users/1','DELETE',204,'"delete success"','','1',NOW(),NOW());
INSERT INTO `test_platform`.mocks (name,`path`,`method`,response_status,response_body,description,project_id,created_at,updated_at) VALUES ('随机数接口示例','/api/example','GET',200,'{"user":{"id":"${uuid}","name":"${name}","email":"${email}","phone":"${phone}","birthday":"${date[%Y-%m-%d,1990-01-01,2000-12-31]}","salary":"${float[5000,20000,2]}","address":"${address}","company":"${company}","avatar":"${image_url}"},"current_time":"${now}","current_date":"${now[%Y-%m-%d]}","timestamp":"${now[%Y%m%d%H%M%S]}"}','','1',NOW(),NOW());

-- 环境示例数据
INSERT INTO `test_platform`.`environments` (id, name, base_url, username, password, description, parameter_count, created_at, updated_at, is_deleted) VALUES(1, 'Dev环境', 'http://www.dev.com', 'admin', '123456', '开发环境', 1, NOW(), NOW(), 0);
INSERT INTO `test_platform`.`environments` (id, name, base_url, username, password, description, parameter_count, created_at, updated_at, is_deleted) VALUES(2, 'Test环境', 'http://www.test.com', 'admin', '123456', '测试环境', 0, NOW(), NOW(), 0);
...
-- 数据库连接示例数据
INSERT INTO `test_platform`.`database_connections` (id, name, host, port, `database`, username, password, driver, charset, description, is_active, created_at, updated_at) VALUES(1, '本地MySQL', '127.0.0.1', 3306, 'test_platform', 'root', '123456', 'mysql', 'utf8mb4', '', 1, NOW(),NOW());
...
-- 脚本管理示例数据
INSERT INTO test_platform.script_management (name, description, script_type, script_content, file_path, is_enabled, timeout_seconds, environment_vars, is_scheduled, cron_expression, schedule_job_id, total_executions, success_count, failure_count, id, created_at, updated_at, is_active) VALUES ('Python脚本示例', '', 'python', '#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
if __name__ == \'__main__\':
    main()
', null, 1, 300, null, 0, '', null, 0, 0, 0, 0, NOW(),NOW(), 1);
INSERT INTO test_platform.script_management (name, description, script_type, script_content, file_path, is_enabled, timeout_seconds, environment_vars, is_scheduled, cron_expression, schedule_job_id, total_executions, success_count, failure_count, id, created_at, updated_at, is_active) VALUES ('Shell脚本示例', '', 'shell', '#!/bin/bash
