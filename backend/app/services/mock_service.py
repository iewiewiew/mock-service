...
from ..utils.utils import DynamicDataProcessor


class MockService:
    """Mock API 服务层"""

    def get_all_mocks(self, page=1, per_page=10, name=None, path=None, method=None, project_id=None):
        ...
        if project_id:
            query = query.filter(Mock.project_id.ilike(f'%{project_id.upper()}%'))

        # 排序和分页
        pagination = query.order_by(Mock.created_at.desc()).paginate(page=page, per_page=per_page, error_out=False)

        return {'data': [mock.to_dict() for mock in pagination.items], 'total': pagination.total}

    def get_mock_by_id(self, mock_id):
        """获取单个 Mock API 详情"""
    ...
    def update_mock(self, mock_id, data):
        """更新 Mock API"""
    ...
    def execute_mock(self, api_path, http_method, request_data):
        """执行 Mock API"""
    ...
    def generate_curl_command(self, api_path, method, host_url):
