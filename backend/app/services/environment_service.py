...
from ..models.environment_model import Environment, EnvironmentParameter


class EnvironmentService:
    def get_all_environments(self, page=1, per_page=10, search=None, project_id=None):
        """获取所有环境（支持分页和搜索）"""

        # 基础查询
        query = Environment.query.filter_by(is_deleted=False)
        ...
        # 排序和分页
        pagination = query.order_by(Environment.updated_at.desc()).paginate(page=page, per_page=per_page,
            error_out=False)
    ...
    def get_environment_by_id(self, environment_id):
        ...
        environment = Environment.query.filter_by(id=environment_id, is_deleted=False).first_or_404()
    ...
    def update_environment(self, environment_id, data):
        """更新环境"""
    ...
    def delete_environment(self, environment_id):
        ...
        try:
            ...
            # 同时软删除关联的所有参数
            EnvironmentParameter.query.filter_by(environment_id=environment_id).update({'is_deleted': True})
    ...
    def get_environment_parameters(self, environment_id, page=1, per_page=10, search=None):
        """获取环境参数列表（支持分页和搜索）"""
        ...
        # 基础查询
        query = EnvironmentParameter.query.filter_by(environment_id=environment_id, is_deleted=False)
        ...
        # 排序和分页
        pagination = query.order_by(EnvironmentParameter.updated_at.desc()).paginate(page=page, per_page=per_page,
            error_out=False)
    ...
    def create_environment_parameter(self, environment_id, data):
        """创建环境参数"""
        ...
        try:
            ...
            # 更新环境的参数计数
            self._update_environment_parameter_count(environment_id)
    ...
    def update_environment_parameter(self, parameter_id, data):
        """更新环境参数"""

        parameter = EnvironmentParameter.query.filter_by(id=parameter_id, is_deleted=False).first_or_404()
    ...
    def delete_environment_parameter(self, parameter_id):
        """删除环境参数（软删除）"""
    ...
    def _update_environment_parameter_count(self, environment_id):
        """更新环境的参数计数"""

        parameter_count = EnvironmentParameter.query.filter_by(environment_id=environment_id, is_deleted=False).count()

        environment = Environment.query.get(environment_id)
        environment.parameter_count = parameter_count

    def get_environment_stats(self, environment_id):
        """获取环境统计信息"""
        ...
        parameter_stats = db.session.query(EnvironmentParameter.param_key,
            db.func.count(EnvironmentParameter.id)).filter_by(environment_id=environment_id, is_deleted=False).group_by(
    ...
    def copy_environment(self, environment_id, new_name):
        """复制环境及其参数"""

        source_environment = Environment.query.filter_by(id=environment_id, is_deleted=False).first_or_404()
        ...
        try:
            ...
            # 更新参数计数
            self._update_environment_parameter_count(new_environment.id)
