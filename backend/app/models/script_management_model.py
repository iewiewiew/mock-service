...
class ScriptManagement(BaseModel):
    """自动化脚本管理模型"""
    __tablename__ = 'script_management'
...
class ScriptExecutionHistory(BaseModel):
    """脚本执行历史记录模型"""
    __tablename__ = 'script_execution_history'

    script_id = db.Column(db.Integer, db.ForeignKey('script_management.id'), nullable=False, comment='脚本ID')
