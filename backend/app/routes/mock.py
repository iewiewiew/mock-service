...
def get_all_mocks():
    ...
    try:
        result = mock_service.get_all_mocks(page=page, per_page=per_page, name=name, path=path, method=method,
...
def get_mock_by_id(mock_id):
    ...
    try:
        mock = mock_service.get_mock_by_id(mock_id)
        return jsonify(mock)
    except Exception as e:
        raise APIException('Server error', 500, {'details': str(e)})
...
def create_mock():
    ...
    try:
        result = mock_service.create_mock(data)
...
def update_mock(mock_id):
    ...
    try:
        result = mock_service.update_mock(mock_id, data)
        return jsonify(result)
    except Exception as e:
        raise APIException('Server error', 500, {'details': str(e)})
...
def delete_mock(mock_id):
    ...
    try:
        result = mock_service.delete_mock(mock_id)
        return jsonify(result)
    except Exception as e:
        raise APIException('Server error', 500, {'details': str(e)})
...
def execute_mock(api_path):
    ...
    try:
        result, status_code = mock_service.execute_mock(api_path, request.method, request_data)
...
def generate_curl_command(api_path):
    ...
    try:
        result = mock_service.generate_curl_command(api_path, method, request.host_url)
        return jsonify(result)
    except APIException as e:
        raise e
    except Exception as e:
