<template>
  <div class="common-list-container">
    ...
    <el-table :data="paginatedServers" style="width: 100%" height="900px" v-loading="loading">
    ...
    >
      ...
      >
        ...
        <el-form-item label="私钥">
          <el-input
            v-model="newServer.private_key"
            type="textarea"
            :rows="4"
            placeholder="请输入SSH私钥内容（cat ~/.ssh/id_rsa 如果使用私钥认证）"
          />
        </el-form-item>
    ...
    >
      ...
      >
        ...
        <el-form-item label="私钥">
          <el-input
            v-model="editServerForm.private_key"
            type="textarea"
            :rows="4"
            placeholder="请输入SSH私钥内容（留空则不修改）"
          />
        </el-form-item>
        <el-form-item label="描述">
          <el-input 
            v-model="editServerForm.description" 
            type="textarea" 
            placeholder="请输入服务器描述" 
          />
        </el-form-item>
        ...
        <div class="quick-commands-bar">
          ...
          <div class="quick-commands-list">
            <el-tag
              ...
              size="small"
              @click="insertQuickCommand(cmd.command)"
            >
              {{ cmd.name }}
            </el-tag>
          </div>
    ...
    <!-- 服务器信息抽屉 -->
    <el-drawer
      v-model="serverInfoDrawerVisible"
      title="服务器信息"
      size="600px"
      direction="rtl"
      destroy-on-close
      @close="handleServerInfoClose"
    >
      <div class="server-info-drawer-container" v-loading="serverInfoLoading">
        <div class="server-info-header">
          <div class="server-basic-info">
            ...
            <p class="server-connection-info">
        ...
        <div class="server-info-content" v-if="store.serverInfo">
          <el-collapse v-model="activeCollapseItems" accordion>
            <el-collapse-item 
              v-for="(value, key) in store.serverInfo" 
              :key="key" 
              :name="key"
        ...
        <div class="server-info-empty" v-else>
          <el-empty description="暂无服务器信息">
            <el-button type="primary" @click="refreshServerInfo" :loading="serverInfoLoading">
              获取服务器信息
            </el-button>
          </el-empty>
        </div>
...
import { useLinuxInfoStore } from '@/stores/linuxInfoStore'
...
const filteredServers = computed(() => {
  ...
  if (searchForm.value.server_name) {
    filtered = filtered.filter(server => 
      server.server_name.toLowerCase().includes(searchForm.value.server_name.toLowerCase())
    )
  }

  if (searchForm.value.host) {
    filtered = filtered.filter(server => 
      server.host.toLowerCase().includes(searchForm.value.host.toLowerCase())
    )
  }
...
const copyTerminalContent = async () => {
  ...
  content += terminalLines.value.map(line => {
