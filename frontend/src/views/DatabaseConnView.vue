      ...
      <div class="search-bar">
        <el-form :inline="true" :model="searchForm" label-width="auto">
          ...
          <el-form-item label="快速选择">
            <el-select
              v-model="quickSelectId"
              placeholder="下拉搜索数据库连接"
              filterable
              clearable
            ...
            >
              <el-option
                ...
                :key="opt.value"
                :label="`${opt.label} (${opt.host}/${opt.database})`"
                :value="opt.value"
              />
            </el-select>
          </el-form-item>
...
const onSelectVisibleChange = async (visible) => {
  ...
  try {
    ...
    await store.fetchConnectionsForSelect()
    connectionOptions.value = store.connectionOptions
  } finally {
    selectLoading.value = false
  }
}

const onQuickSelectChange = async (id) => {
  if (!id) return
  const conn = store.connections.find(c => c.id === id)
  if (conn) {
    searchForm.value.keyword = conn.name
    currentPage.value = 1
    await loadList()
  }
}
