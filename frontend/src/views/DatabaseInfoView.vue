    ...
    <div class="header-bar">
      ...
      >
        <el-option
          ...
          :key="conn.value"
          :label="`${conn.label} (${conn.host})`"
          :value="conn.value"
        />
      </el-select>
      <el-button v-if="selectedConnectionId" @click="loadDatabases" :loading="store.loading">
        刷新
      </el-button>
      <el-button 
        v-if="selectedConnectionId" 
        type="primary" 
        @click="handleExportDatabasesSql"
        :loading="exportDatabasesLoading"
      >
            ...
            <div v-if="store.currentTable">
              <div class="table-header">
                <span class="table-name">
                  数据库: {{ store.currentDatabase }} / 表: {{ store.currentTable }}
                </span>
                <div style="display: flex; align-items: center; gap: 12px;">
                  <span style="color:#606266;font-size:12px;">
                    共 {{ store.pagination.total }} 条
                    <span v-if="selectedRows.length > 0" style="color: #409EFF; margin-left: 8px;">
          ...
          <el-tab-pane label="SQL控制台" name="console">
            <div class="console-container">
              <div class="console-toolbar">
                <el-button size="small" type="primary" @click="executeQuery" :loading="store.loading">
                  执行查询
                </el-button>
              ...
              <div class="editor-area">
                <el-input
                  ...
                  type="textarea"
                  :rows="8"
                  placeholder="请输入SQL查询语句，例如：SELECT * FROM users LIMIT 10"
                />
              </div>
      ...
      <div style="margin-bottom: 20px; max-height: 400px; overflow-y: auto; border: 1px solid #e4e7ed; padding: 12px; border-radius: 4px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
          ...
          <div style="display: flex; gap: 8px;">
            <el-button 
              ...
              text 
              type="primary" 
              @click="selectAllDatabasesTables"
        ...
        <div v-for="db in databaseTablesForExport" :key="db.name" style="margin-bottom: 16px;">
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
            <el-checkbox 
              :model-value="db.selected"
              @change="(val) => handleDatabaseSelectChange(db.name, val)"
              :indeterminate="db.indeterminate"
            >
          ...
          <div v-if="db.expanded" style="margin-left: 24px; padding-left: 16px; border-left: 2px solid #e4e7ed;">
            ...
            <div v-else-if="db.tables.length === 0" style="padding: 8px; color: #909399;">
              该数据库暂无表
            </div>
            <div v-else style="display: flex; flex-direction: column; gap: 6px;">
              <el-checkbox 
                ...
                :key="table"
                :model-value="db.selectedTables.includes(table)"
                @change="(val) => handleTableSelectChange(db.name, table, val)"
              >
                {{ table }}
              </el-checkbox>
            </div>
...
import { useDatabaseInfoStore } from '@/stores/databaseInfoStore'
import { useDatabaseConnStore } from '@/stores/databaseConnStore'
import { formatDateTime } from '@/utils/date'
import { databaseInfoService } from '@/services/databaseInfoService'
...
const tableColumns = computed(() => {
  ...
  const sortedKeys = [
    ...keys.filter(k => k === 'id'),
    ...keys.filter(k => !['id', 'created_at', 'updated_at'].includes(k)),
    ...keys.filter(k => ['created_at', 'updated_at'].includes(k))
  ]
  return sortedKeys
})
...
const loadDatabases = async () => {
  ...
  try {
    ...
    ElMessage.error(store.error || '加载数据库列表失败')
  }
}
...
const confirmExportSql = async () => {
  ...
  try {
    ...
    const response = await databaseInfoService.exportDataToSql(
...
const confirmExportDatabasesSql = async () => {
  ...
  try {
    ...
    const response = await databaseInfoService.exportDatabasesToSql(
      selectedConnectionId.value,
      {
