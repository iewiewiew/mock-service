<template>
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; padding: 0 4px;">
        <div style="display: flex; align-items: center; gap: 12px;">
          <span style="color: #606266; font-size: 14px;">
            已选择 <span style="color: #409EFF; font-weight: 500;">{{ selectedIds.size }}</span> 项
          </span>
          <el-button 
            text 
            type="primary" 
  ...
  const updateTableSelection = async () => {
    ...
    // 收集需要选中的行ID
    const idsToSelect = tableData.value
      .filter(row => row && row.id !== undefined && selectedIds.value.has(row.id))
      .map(row => row.id)
    
    console.log('updateTableSelection - 需要选中的行数:', idsToSelect.length, 
                '当前页数据:', tableData.value.length, 
                'selectedIds数量:', selectedIds.value.size,
                '当前页的ID列表:', tableData.value.map(r => r.id),
                '应该选中的ID:', idsToSelect)
    ...
    try {
      if (tableRef.value && typeof tableRef.value.getSelectionRows === 'function') {
        ...
        console.log('updateTableSelection - 实际选中的行数:', selectedRows?.length || 0, 
                    '选中的ID:', selectedRows?.map(r => r.id) || [])
      }
    } catch (error) {
    ...
    try {
      ...
      for (let page = 1; page <= totalPages; page++) {
        try {
          ...
          // exampleService.getExamples返回格式: { data: { data: [...], total: ... }, total: ... }
          // store中使用的是 response.data.data，所以这里也应该用 response.data.data
      ...
      console.log('全选完成 - 当前页应选中:', currentPageSelected, '条，实际数据:', tableData.value.length, 
                  'selectedIds数量:', selectedIds.value.size)
