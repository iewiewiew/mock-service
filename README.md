...
```
查询所有
curl -X GET "http://localhost:5001/api/mock"

分页查询
curl -X GET "http://localhost:5001/api/mock?page=1&per_page=10"

获取单个 Mock 数据
curl -X GET "http://localhost:5001/api/mock/1"
```

- 新增接口

```
curl -X POST "http://localhost:5001/api/mock" \
...
response_body 示例
{"request_id":"{request.json.id}","req_str":"{request.json.req_str}","action":"{request.args.action}","id_card":"${id_card}","email":"${email}","name":"${name}","example":"示例"}
```

- 更新接口

```
curl -X PUT "http://localhost:5001/api/mock/1" \
-H "Content-Type: application/json" \
-d '{"name": "Updated API", "response_status": 200, "response_body": "{'message": "Updated'}"}'
```

- 删除接口

```
curl -X DELETE "http://localhost:5001/api/mock/1"
```
...
```
测试 GET 接口
curl -X GET "http://127.0.0.1:5001/api/mock/execute/api/users" | jq .
curl -X GET "http://127.0.0.1:5001/api/mock/execute/api/example" | jq .

测试 POST 接口
curl -X POST "http://127.0.0.1:5001/api/mock/execute/api/users?action=create" \
  -H "Content-Type: application/json" \
  -d '{"id": 123456, "req_str": "请求的字符串"}' | jq .

测试 PUT 接口
curl -X PUT "http://127.0.0.1:5001/api/mock/execute/api/users/1" \
  -H "Content-Type: application/json" \
  -d '{"name": "Jane"}' | jq .

测试 DELETE 接口
curl -X DELETE "http://127.0.0.1:5001/api/mock/execute/api/users/1"
```
